import { Link, Text, Box, Grid, GridItem, Stack, Table, Thead, Tbody, Th, Tr, Td, Tabs, Tab, TabList, TabPanels, TabPanel, Skeleton, Tooltip } from '@chakra-ui/react'
...
const Epoch = () => {
  ...
  const { data: epoch, isLoading: isEpochLoading, isError: isEpochError } = useQuery({
    cacheTime: Infinity,
    queryKey: ['vsc-epoch', epchNum],
    queryFn: async () => fetchEpoch(epchNum),
    enabled: !invalidEpochNum
  })
  const { data: blocks, isLoading: isBlocksLoading, isError: isBlocksError } = useQuery({
    cacheTime: 60000,
        ...
        <Box>
          <Table mt={'20px'}>
            <Tbody>
              ...
              <TableRow label='Timestamp' value={epoch ? epoch.ts+' ('+timeAgo(epoch.ts)+')' : ''} isLoading={isEpochLoading}/>
              <TableRow label='L1 Tx' value={epoch?.l1_tx} isLoading={isEpochLoading} link={'/tx/'+epoch?.l1_tx}/>
              <TableRow label='L1 Block' value={epoch?.l1_block_num} isLoading={isEpochLoading} link={l1Explorer+'/b/'+epoch?.l1_block_num}/>
          ...
          <Tabs mt={'7'} colorScheme={themeColorScheme} variant={'enclosed'}>
            ...
            <TabPanels mt={'2'}>
              <TabPanel>
                {isBlocksLoading ? <Skeleton height={'20px'}/> : (
