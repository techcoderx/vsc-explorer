...
const AnchorRef = (
  anchorRef: AnchorRefResult,
  isRefLoading: boolean,
  isRefError: boolean,
  invalidRefNum: boolean,
  refNum: number
) => {
  if (invalidRefNum) return <PageNotFound />
  return (
    <>
      ...
      <hr />
      {anchorRef?.error || isRefError ? (
        <Text>Failed to load anchor ref, error: {anchorRef ? anchorRef.error : 'Failed to fetch from HAF node'}</Text>
      ) : (
        <Box>
          <Table mt={'20px'}>
            <Tbody>
              ...
              <TableRow
                label="Timestamp"
                value={anchorRef ? anchorRef.ts + ' (' + timeAgo(anchorRef.ts) + ')' : ''}
                isLoading={isRefLoading}
              />
              <TableRow
                label="L2 Block"
                value={anchorRef?.block_num}
                link={'/block/' + anchorRef?.block_num}
                isLoading={isRefLoading}
              />
          ...
          <Heading mt={'7'} fontSize={'xl'}>
            {(anchorRef && anchorRef.refs.length) ?? 0} transaction{isPuralArr((anchorRef && anchorRef.refs) ?? []) ? 's' : ''} in
            this anchor ref
          </Heading>
          <Flex direction={'column'} gap={'3'} mt={'3'}>
            {anchorRef?.refs.map((tx, i) => {
              return (
                <TxCard key={i} id={i} ts={anchorRef?.ts} txid={tx}>
                  {tx}
                </TxCard>
              )
            })}
