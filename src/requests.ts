    ...
    queryFn: async () => {
      return (
        await gql<GqlResponse<AddrBalance>>(
          `query AccBal($acc: String!) { bal: getAccountBalance(account: $acc) { hbd hbd_savings hive hive_consensus consensus_unstaking pending_hbd_unstaking } rc: getAccountRC(account: $acc) { amount max_rcs }}`,
          { acc }
        )
      ).data
    }
...
export const fetchL2TxnsDetailed = async (id: string): Promise<{ txns: Txn[] }> => {
  const result = await gql<GqlResponse<{ txns: Txn[] }>>(
    `query AccHistory ($opts: TransactionFilter) { txns: findTransaction(filterOptions: $opts) { id anchr_height anchr_ts required_auths nonce status ops { type, index, data } rc_limit ledger { type from to amount asset memo params } ledger_actions { type status to amount asset memo data } output { id index } }}`,
    {
      opts: {
        byId: id,
        offset: 0,
